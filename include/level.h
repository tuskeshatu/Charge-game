...
extern const float windoHeight;
...
public:
    Level(const std::string &levelName = "empty_level", const sf::Vector2u &levelSize = sf::Vector2u(windowWidth, windowHeight), const std::vector<Obstacle> &obstacles = std::vector<Obstacle>(), const sf::Vector2f &playerStartPos = sf::Vector2f(windowWidth / 2, windowHeight / 2));
    ~Level() {}
